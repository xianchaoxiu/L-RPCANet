...
#函数用于计算给定数据集的性能指标，如 FPR、TPR 和 AUC。它接受数据集名称、bins数量和文件名称作为参数。函数内部初始化数据集、评估指标计算器，并遍历数据集中的所有样本，更新评估指标计算器。最后，它打印并保存评估结果。
...
#在主执行块中，脚本设置了一些参数，如特定数据集列表、结果文件目录和文件名。然后，对于列表中的每个数据集，它调用 cal_fpr_tpr 函数来计算和保存性能指标。
